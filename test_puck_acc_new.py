...
class PuckAccuracyTester:
    ...
    def index_training_vectors(self, index_name: str, vectors: np.ndarray) -> None:
        ...
        success, failed = bulk(self.client, actions, chunk_size=len(actions), refresh=False)
    ...
    def diagnose_index(self, index_name: str) -> None:
        """Diagnose index state and document count"""
        print(f"\n=== Diagnosing index: {index_name} ===")
        
        # Check document count
        count_response = self.client.count(index=index_name)
        print(f"Document count: {count_response['count']}")
        
        # Check index stats
        stats = self.client.indices.stats(index=index_name)
        print(f"Index stats: {json.dumps(stats['indices'][index_name]['total'], indent=2)}")
        
        # Check index settings
        settings = self.client.indices.get(index=index_name)
        print(f"Index settings: {json.dumps(settings[index_name]['settings'], indent=2)}")
        
        # Try to retrieve a sample document
        try:
            sample = self.client.get(index=index_name, id="0")
            print(f"Sample document exists: {sample['found']}")
            if sample['found']:
                print(f"Sample doc has vector: {'vector' in sample['_source']}")
        except Exception as e:
            print(f"Error retrieving sample doc: {e}")
        
        # Try a match_all query
        try:
            match_all = self.client.search(index=index_name, body={"query": {"match_all": {}}, "size": 1})
            print(f"Match all query returned: {match_all['hits']['total']['value']} docs")
        except Exception as e:
            print(f"Error with match_all query: {e}")
    ...
    def index_vectors(self, index_name: str, vectors: np.ndarray) -> float:
        """Fixed indexing with verification"""
        start_time = time.time()
        
        # Index in smaller batches to avoid timeouts
        batch_size = 100
        total_indexed = 0
        ...
        for i in range(0, len(vectors), batch_size):
            ...
            if failed:
                print(f"Failed to index {failed} documents in batch {i//batch_size}")
        
        # Refresh once at the end
        self.client.indices.refresh(index=index_name)
        
        # Verify indexing
        final_count = self.client.count(index=index_name)
        print(f"Indexed {total_indexed} vectors, verified count: {final_count['count']}")
        
        if final_count['count'] != len(vectors):
            raise Exception(f"Indexing mismatch: expected {len(vectors)}, got {final_count['count']}")
    ...
    def search_knn(
        ...
        query = {
            ...
            "query": {
                "knn": {
                    field_name: {  # Use the correct field name
    ...
    def load_model_explicitly(self, model_id: str) -> None:
        """Load the Puck model into cache for searching"""
        print(f"\n=== Loading Model {model_id} into Cache ===")
        
        # Try to load the model
        try:
            # Some OpenSearch versions use _load endpoint
            response = self.client.transport.perform_request(
                'POST',
                f'/_plugins/_knn/models/{model_id}/_load',
                body={}
            )
            print(f"Load response: {json.dumps(response, indent=2)}")
        except Exception as e:
            print(f"Load endpoint not available or failed: {e}")
            print("This is normal for some OpenSearch versions")
        
        # Wait for model to be ready
        print("Waiting 15 seconds for model to load...")
        time.sleep(15)
        
        # Check if model stats show it's loaded
        try:
            stats = self.client.transport.perform_request(
                'GET',
                '/_plugins/_knn/_stats'
            )
            print(f"\nKNN Stats:")
            print(json.dumps(stats, indent=2))
        except Exception as e:
            print(f"Could not get KNN stats: {e}")
    ...
    def run_accuracy_test(
        ...
        # train_index = np.random.choice(len(database_vectors), num_train)
        train_vectors = database_vectors
        ...
        training_index = "puck_train_index_6"
        ...
        self.diagnose_index(test_index)
        ...
        for i, query_vec in enumerate(query_vectors):
            # Search the test index (with model), not training index
            result_ids, search_time = self.search_knn(test_index, query_vec, k, field_name="vector")
...
def main():
    ...
    parser.add_argument('--k', type=int, default=100, help='K for top-K search')
    parser.add_argument('--coarse-clusters', type=int, default=100, help='Number of coarse clusters')
    parser.add_argument('--fine-clusters', type=int, default=100, help='Number of fine clusters')
